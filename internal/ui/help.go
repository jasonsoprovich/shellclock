...
func (m *HelpModel) buildLines() {
	...
	blank()
	add(h2("Timer"))
	add(row("s", "Start timer on focused task / pause / resume"))
	...
	blank()
	add(note("CSV export is session-level: Project, Task, Start, End, Duration, Notes"))
	...
	blank()
	add(row("B", "Show backup info overlay (from tree view)"))
	blank()
