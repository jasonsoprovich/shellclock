...
func (m *HelpModel) buildLines() {
	...
	blank()
	add(h2("Timer"))
	add(row("s", "Start / pause / resume timer on focused task"))
	...
	add(row("v", "Open summary view (today's sessions)"))
	...
	add(h2("Summary View"))
	add(sep())
	add(note("Open with v from the tree view. Shows sessions logged today or this week."))
	blank()
	add(row("↑ / k", "Scroll up"))
